...
/*
    클라이언트
*/
const userTaleList = document.querySelector('.user-table-list');
...
function load() {
    ...
    for(let i = 0; i <userListObj.length; i++) {
        userTaleList.appendChild(createTr(userListObj[i]));
    }

    for(let i = 0; i <userListObj.length; i++) {
        userTaleList.appendChild(createTr2(userListObj[i]));
    }

    for(let i = 0; i <userListObj.length; i++) {
        userTaleList.innerHTML += (createTr3(userListObj[i]));
    }
